...
void CVideoEncoder::SetVideoHeightAndWidth(BITMAP bitmap) {
    mVideoHeight = bitmap.bmHeight;
    mVideoWidth = bitmap.bmWidth;
}

template <class T> void CVideoEncoder::SafeRelease(T **ppT)
{
    if (*ppT)
    {
        (*ppT)->Release();
...
UINT64 CVideoEncoder::GetVideoFrameDuration(){
    return 10 * 1000 * 1000 / mVideoFps;
}

UINT64 CVideoEncoder::GetVideoFrameCount() {
    return mDuration * mVideoFps;
}

HRESULT CVideoEncoder::WriteFrame(
...
HRESULT CVideoEncoder::StartMediaFoundation() {
...
HRESULT CVideoEncoder::InitializeSinkWriter(IMFSinkWriter **ppWriter, DWORD *pStreamIndex, DWORD *pAudioStreamIndex, LPCWSTR videoOutputFilePath, IMFSourceReader *pReader)
{
    *ppWriter = NULL;
    *pStreamIndex = NULL;
...
HRESULT CVideoEncoder::GetSourceDuration(IMFMediaSource *pSource, MFTIME *pDuration)
{
    *pDuration = 0;
...
HRESULT CVideoEncoder::CreateMediaSource(
    PCWSTR pszURL,
...
bool CVideoEncoder::ConvertToDIB(HBITMAP& hBitmap) {
...
HBITMAP CVideoEncoder::LoadImageFromFilePath(LPCWSTR filePath) {
    return (HBITMAP)LoadImage(NULL, filePath, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
}

int CVideoEncoder::GetDIBFromHandle(HBITMAP hBitmap, BITMAP *bitmap) {
    bool success = ConvertToDIB(hBitmap);
    int errorCode = GetObject(hBitmap, sizeof(BITMAP), bitmap);
    return errorCode;
}

STDMETHODIMP CVideoEncoder::Encode()
{
